...
app.config['SECRET_KEY'] = "r@nd0m_s3cr3t"
...
def select_metrics(metrics):
    ...
    for metric in metrics:
        if metric == "mae":
            ...
            metrics_chosen.append(cornac.metrics.Recall(k=20))
        elif metric == "precision":
            metrics_chosen.append(cornac.metrics.Precision(k=20))
...
def run_model(parameter):
    ...
    if request.method == "GET":
        ...
        if len(form_files) == 0:
            errors.append("Please ensure both dataset and meta data files are uploaded")
        ...
        metrics = select_metrics(inputParam["metrics"])
